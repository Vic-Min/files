...
static void setDepthStencilState(vsg::ref_ptr<vsg::Node>& quad)
{
    if (const auto& stateGroup = quad.cast<vsg::StateGroup>())
    {
        for (const vsg::ref_ptr<vsg::StateCommand>& stateCommand : stateGroup->stateCommands)
        {
            if (const auto& graphicsPipeline = stateCommand.cast<vsg::BindGraphicsPipeline>())
            {
                if (const vsg::ref_ptr<vsg::GraphicsPipeline>& pipeline = graphicsPipeline->pipeline)
                {
                    for (const vsg::ref_ptr<vsg::GraphicsPipelineState>& pipelineState : pipeline->pipelineStates)
                    {
                        if (const auto& depthStencilState = pipelineState.cast<vsg::DepthStencilState>())
                        {
